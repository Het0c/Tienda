from PyQt5.QtWidgets import (
    ...
    QPushButton, QDialog, QLineEdit, QHBoxLayout, QMessageBox
)
...
class VentanaDetalle(QDialog):
    ...
    def guardar_cambios(self):
        ...
        if confirmacion == QMessageBox.Yes:
            print(f"Contraseña guardada: {self.input_password.text()}")  # Aquí irá la lógica de BD
            self.close()

    def ver_password_temporal(self):
        self.input_password.setEchoMode(QLineEdit.Normal)
        QTimer.singleShot(2000, lambda: self.input_password.setEchoMode(QLineEdit.Password))
...
def crear_pagina_clientes():
    ...
    def agregar_fila(nombre, rut, cargo):
        ...
        btn_accion.setStyleSheet("background-color: #dfe6e9; border-radius: 4px; font-weight: bold;")
        tabla.setCellWidget(fila, 3, btn_accion)

        btn_accion.clicked.connect(lambda _, f=fila: VentanaAutorizacion(tabla, f).exec_())
